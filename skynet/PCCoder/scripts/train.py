...
def dataset_shuffle(data, size):
    ...
    shuffled_index = np.random.permutation(len(size))
...
def train(args):
    ...
    data, statement_target, drop_target, operator_target = tf.convert_to_tensor(data), \
    ...
    data = tf.gather(data, shuffled_indices)
    statement_target = tf.gather(statement_target, shuffled_indices)
    drop_target = tf.gather(drop_target, shuffled_indices)
    operator_target = tf.gather(operator_target, shuffled_indices)
    ...
    data_loader = butch_loader(data=(train_data, train_statement_target, train_drop_target, train_operator_target),
                               dataset_size=train_size, batch_size=batch_size)

    model: PCCoder = PCCoder("PCCoder")
    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(range(0, train_size, batch_size)):
            ...
            with tf.GradientTape() as tape:
                ...
                # on the GradientTape.
                pred_act, pred_drop, pred_operator = model(x, training=True)  # Logits for this minibatch
