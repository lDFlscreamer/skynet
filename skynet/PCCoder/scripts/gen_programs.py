...
from skynet.DeepCoder.dsl import DeepCoder_constraint as constraint
from skynet.DeepCoder.dsl.DeepCoder_program import Program, get_used_indices, get_unused_indices
from skynet.DeepCoder.dsl.DeepCoder_types import INT, LIST
from skynet.DeepCoder.dsl.DeepCoder_value import Value
from skynet.DeepCoder.dsl.impl import function_pool
...
def gen_program_worker(input_types):
    ...
    def helper(functions, program, programs):
        ...
        for func in function_pool.LAMBDAS:
    ...
    while progress_counter.value < num_programs:
        helper(function_pool.ALL_FUNCTIONS, program_base, res)
    return res
...
def gen_programs(program_len, num_programs, args):
    ...
    input_type_combinations = get_input_type_combinations(params._num_inputs)
...
def load_cache(path):
    ...
    for i, line in enumerate(lines):
        ...
        p_examples = Example.from_line(line, Value)
...
def main():
    ...
    parser.add_argument('--num_examples', type=int, default=params._num_examples)
    ...
    with open(args.train_output_path, 'a') as f:
